...
def drawPath(path,robot,color):
	...
	for i in path:
		...
		draw.append(env.plot3(points=robot.GetTransform()[0:3,3],pointsize=0.1,colors=color,drawstyle=1))
...
if __name__ == "__main__":
	...
	env.Load('data/quadrotor.env.xml')
	time.sleep(0.1)

	# 1) get the 1st robot that is inside the loaded scene
	# 2) assign it to the variable named 'robot'
	#env.Load('data/quadrotor.robot.xml')	
...
#	tuckarms(env,robot);
	...
	with env:
		goalconfig = [-22.4072,-21.3722,14.2062]
		...
		path = RRTmodule.SendCommand('RRT goal %f,%f,%f; goalBias 0.15; step %f; metricWeight 1000,1000,1000; shortCutStep 0; shortIters %f; biRRTFlag %f;'%tuple(goalconfig+[stepSize,ShortCutIters,biRRTFlag]))
		print path		
