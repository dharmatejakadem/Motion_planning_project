...
public:
    ...
    RRTmodule(EnvironmentBasePtr penv, std::istream& ss) : ModuleBase(penv) {
        RegisterCommand("RRT",boost::bind(&RRTmodule::MyCommand,this,_1,_2),
                        "Input format: RRT goal %f,%f,%f,%f,%f,%f; goalBias 0.1; step .2; metricWeight 900,800,60,25,0,13,0; shortCutStep 0.15; biRRTFlag 0;");
    ...
    {vector<RobotBasePtr> vbodies;
        GetEnv()->GetRobots(vbodies);
        RobotBasePtr robot;
        robot = vbodies[0];
        vector<double> start;

        robot->GetActiveDOFValues(start);
     path = EGRRTbuild(start,goalConfig,GetEnv());}
